...
if __name__=='__main__':
    ...
    crtDir = os.getcwd()
    if len(sys.argv)==1:
        filePath = os.path.join(crtDir, "Input", 'v1_world-happiness-report-2017.csv')
    else:
        filePath = os.path.join(crtDir, "Input", sys.argv[1])

    x, y = loadData(filePath, "Economy..GDP.per.Capita.", "Freedom", "Happiness.Score")

    train_x,train_y, test_x, test_y = split_train_and_test(x,y)
    ...
    X, Y = np.meshgrid(range(math.floor(min(t1)-0.5),math.ceil(max(t1)+0.5)), range(math.floor(min(t2)-0.5),math.ceil(max(t2)+0.5)))
    Z = w0+w1*X+w2*Y
