...
namespace AI.Lab9.Algebra
{
    public class Matrix
    {
        ...
        public override string ToString() => ToString(8);
        ...
        public string ToString(int decimals)
        {
            ...
            for (int i = 0; i < RowsCount; i++)
            {
                ...
                {
                    if (j > 0) s += " ";
                    s += Data[i, j].ToString($"F{decimals}").PadLeft(4 + decimals);
                }
                s += "\n";
            }
            return s;
        ...
        public static Matrix ColumnOf(double[] c)
        {
            return new Matrix(c.Length, 1, c);
        }

        public Matrix PrependColumnOfValues(double c)
        {
            var m = new Matrix(RowsCount, ColsCount + 1);
        ...
        public static Matrix FromColumns(IEnumerable<double[]> _columns)
        {
            var columns = _columns.ToArray();

            int rowsCount = columns.First().Length;
