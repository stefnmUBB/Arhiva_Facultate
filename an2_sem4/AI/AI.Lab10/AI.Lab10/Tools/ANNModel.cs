...
namespace AI.Lab10.Tools
{
    ...
    {        
        ...
        public List<LayerData> HiddenLayers { get; set; } = new List<LayerData>();
        ...
        public static NeuronGenerator RandomGenerator() => NeuronGenerators.All[Rand.Int(NeuronGenerators.All.Count)]();

        public NeuralNetwork GetANN()
        {
            var ann = new NeuralNetwork() { IterationsCount = 2, BatchSize = BatchSize, LearningRate=LearningRate };
    ...
    public class LayerData
    {
        ...
        public LayerData(int neuronsCount, NeuronGenerator generator, params double[] vals)
        {
            NeuronsCount = neuronsCount;
            Generator = generator;
            //Console.WriteLine($"{Generator.Params.Length}--{vals.Length}");
            for (int i = 0; i < Generator.ParamsCount; i++)
                Generator.Params[i] = vals[i];
        }
    ...
    public class NeuronGenerator
    {
        ...
        public NeuronGenerator Clone() => new NeuronGenerator(Name, Functor, Params.ToArray());
    }
    ...
    internal static class NeuronGenerators
    {
        ...
        public static NeuronGenerator CubicActivated() => new NeuronGenerator("Cubic",
            (v) => Neurons.Hidden(ActivationFunction.Cubic(v[0], v[1], v[2], v[3])), 4);

        public static NeuronGenerator QuadActivated() => new NeuronGenerator("CQuad",
            (v) => Neurons.Hidden(ActivationFunction.Quad(v[0], v[1], v[2])), 3);
