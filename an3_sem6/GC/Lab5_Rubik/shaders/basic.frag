...
{				
	...
	if(vertColor.x<0.1 && vertColor.y<0.1 && vertColor.z<0.1) 
		FragColor = vertColor;
	else
	{
		vec4 color = (texture(texImage, TexCoord)*0.5+vertColor*0.5);
		float x = TexCoord.x-0.5, y=TexCoord.y-0.5, r=0.47;		
		int n=10;
		if(pow(x,n)+pow(y,n)>pow(r,n))
			color = vec4(0,0,0,1);
